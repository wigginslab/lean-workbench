...
class StoreCrunchbase:
	...
	def get_people(self):
		...
		for person in people_list:
			...
			if person == "error":
				self.error_file.write(person['permalink'])
	...
	def save_companies(self, company_list, company_type):
		...
		for company_index in range(0, len(company_list)):
			...
			if company != "error":
				...
				self.store_funding(company['funding_round'])
	...
	def store_funding(funding_rounds):
		...
		for investment in funding_round['investments']:
			...
			for investor in investment:
				investor_keys = investor.keys()
				# if investor is a person
				if "person" in investor_keys:
					...
					# if person already exists in the database
								# if investor is financial org
				elif "financial_org" in investor:
					investor = Company.query.filter_by(name=investor['financial_org']['name'])
				# investor is a company
				else:
					investor = Company.query.filter_by(name=investor['company']['name'])
				if investor:
					investors.append(investor)
				else:
					self.error_file.write(investment);
